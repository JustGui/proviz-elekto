...
- **Aggregator auto-sync** - OpenRouter, Requesty, **Nous Portal**, **OrcaRouter** and **TokenHub** each aggregate hundreds of models with drifting pricing; their `models.json` is machine-generated (once at startup, then hourly). `proviz providers sync-<name> --dry-run` to review the mapping. OrcaRouter's `/v1/models` omits capability flags and cache pricing, so each newly-seen model is additionally scraped once from its SEO detail page (`PROVIZ_ORCAROUTER_ENRICH_MAX` per run, fail-soft); its real per-request cost comes back as `usage.cost_usd` when `/complete` sends the `X-OrcaRouter-Include-Cost` header, and it gets `reasoning_effort: "minimal"` by default since it has no reasoning-off switch. TokenHub's authenticated `/v1/models` returns only model ids, so each model's pricing (one-api-style `model_ratio` × $2/1M), context and capabilities is scraped from its server-rendered detail page; known models are carried forward untouched, `proviz providers sync-tokenhub --refresh` re-scrapes everything. Its real per-request cost comes back as `usage.cost` (handled generically, no brand-specific code).
...
- **Prompt-cache stickiness** - `proviz group set-sticky --slug <g> --enabled` makes the selector nudge consecutive calls for that group toward the model that served the last one, so a large repeated prompt prefix stays cache-warm. A bounded bonus that yields to heatroom: it can't keep a rate-limited or headroom-drained model winning, so rotation under load is unaffected. In a sticky group the `cost` score also blends toward the cached input rate (repeat calls hit the cache), favouring cache-capable models.
